#!/usr/bin/env python3
"""
YouTube Automation - CHANNEL VIEW MODE (Refactored)
Uses common.search for performing searches, and common.find for clicking results.
No fallback to direct URL.
"""
...
def create_driver(cfg: SessionConfig):
    ...
    profile_dir = os.path.join(os.getcwd(), f"yt_channel_cache_{cfg.instance_id}_{timestamp}")
...
def run_session(cfg: SessionConfig):
    ...
    try:
        ...
        # Step 1: Perform search for channel name (no result clicking)
        if cfg.is_mobile:
            if not MobileSearch.perform_search(driver, cfg.instance_id, cfg.channel_name):
                logger.error(f"Instance {cfg.instance_id}: Mobile search for channel failed")
                return
        else:
            if not DesktopSearch.perform_search(driver, cfg.instance_id, cfg.channel_name):
                logger.error(f"Instance {cfg.instance_id}: Desktop search for channel failed")
                return

        # Step 2: Find and click channel result
        if not find_and_click_channel_result(driver, cfg.instance_id, cfg.channel_name, cfg.is_mobile):
            logger.error(f"Instance {cfg.instance_id}: Could not click channel result")
            return

        # Step 3: Inside channel, search for video ID using channel_internal_search
        if not channel_internal_search(driver, cfg.instance_id, cfg.video_id, cfg.is_mobile):
            logger.error(f"Instance {cfg.instance_id}: Channel internal search failed, aborting")
            return
