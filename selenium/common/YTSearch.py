...
print(f"[DEBUG] COMMON_ROOT exists: {COMMON_ROOT.exists()}")
print(f"[DEBUG] human_behavior.py exists: {(COMMON_ROOT / 'human_behavior.py').exists()}")
print(f"[DEBUG] sys.path entries with 'common': {[p for p in sys.path if 'common' in p]}")
...
try:
    from common.human_behavior import (
        ...
        ensure_video_playback, handle_all_popups
    )
    print(f"[DEBUG] Imported human_behavior from common package")
except ImportError as e:
    print(f"[DEBUG] Package import failed: {e}, trying direct file load...")
    # Fallback: load directly from file
    human_behavior_path = COMMON_ROOT / "human_behavior.py"
    if human_behavior_path.exists():
        spec = importlib.util.spec_from_file_location("human_behavior", human_behavior_path)
        human_behavior_module = importlib.util.module_from_spec(spec)
        spec.loader.exec_module(human_behavior_module)
        watch_with_human_behavior = human_behavior_module.watch_with_human_behavior
        start_video_with_audio_mute = human_behavior_module.start_video_with_audio_mute
        click_suggested_video = human_behavior_module.click_suggested_video
        ensure_video_playback = human_behavior_module.ensure_video_playback
        handle_all_popups = human_behavior_module.handle_all_popups
        print(f"[DEBUG] Loaded human_behavior from file: {human_behavior_path}")
    else:
        raise ImportError(f"Cannot find human_behavior module at {human_behavior_path}")
)
